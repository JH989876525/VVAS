...
#include "vvas_xpostsegmentation.hpp"
...
int drawOverlaySegmentationMapYUV(vvas_xoverlaypriv *kpriv, Mat segResult)
{
    ...
    {
        LOG_MESSAGE (LOG_LEVEL_ERROR, kpriv->log_level, "Not VVAS_VFMT_Y_UV8_420");
        return -1;
    }
...
int drawOverlaySegmentationC1MapYUV(vvas_xoverlaypriv *kpriv, Mat segResult)
{
...
int drawOverlaySegmentationMapBGR(vvas_xoverlaypriv *kpriv, Mat segResult)
{
    if(kpriv->frameinfo.en_overlay == false)
    {
      return 0;
    ...
    struct overlayframe_info *frameinfo = &(kpriv->frameinfo);
    if(frameinfo->inframe->props.fmt != VVAS_VFMT_BGR8)
    {
        LOG_MESSAGE (LOG_LEVEL_ERROR, kpriv->log_level, "Not VVAS_VFMT_BGR8");
        return -1;
    }
    cv::Mat img_c3;
...
int drawOverylayTextInfoYUV(vvas_xoverlaypriv *kpriv, char*label_string){
...
void writeResult2File(vvas_xoverlaypriv *kpriv,int *results){
...
int classifyScenariosfromSegC1(vvas_xoverlaypriv *kpriv,Mat segResult){
...
{       
    ...
    // JHH  XkprivGetJsonData_int are not sure yet
    ...
    int baseline;
    Size textsize;
    textsize = getTextSize ("A", kpriv->font, kpriv->font_size, 1, &baseline); // JH 不確定 A 
        ...
        VVASFrame *input[MAX_NUM_OBJECT], VVASFrame *output[MAX_NUM_OBJECT])
{
    ...
    //标记一下数据类型
    if(frameinfo->inframe->props.fmt== VVAS_VFMT_BGR8){
        LOG_MESSAGE (LOG_LEVEL_INFO, kpriv->log_level, "FRAME FMT: VVAS_VFMT_BGR8");
    }
    else if(frameinfo->inframe->props.fmt == VVAS_VFMT_Y_UV8_420)
    {
        LOG_MESSAGE (LOG_LEVEL_INFO, kpriv->log_level, "FRAME FMT: VVAS_VFMT_Y_UV8_420");
    }
    else
    {
        LOG_MESSAGE (LOG_LEVEL_INFO, kpriv->log_level, "FRAME FMT: %d",frameinfo->inframe->props.fmt);
    }
